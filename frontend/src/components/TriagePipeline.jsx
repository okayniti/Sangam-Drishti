// ═══════════════════════════════════════════════════════════════════════════════
// TRIAGE PIPELINE — Three-column Kanban incident command matrix
// Column A: UNASSIGNED TRIAGE — incidents awaiting attention (critical items flash)
// Column B: ACTIVE DISPATCH — incidents with assigned volunteers traveling/working
// Column C: RESOLVED LOGS — mitigated incidents with archival timestamps
// ═══════════════════════════════════════════════════════════════════════════════
...
const PRIORITY_STYLES = {
  ...
  ROUTINE: {
    card: 'bg-indigo-500/8 border-indigo-500/25',
...
export default function TriagePipeline() {
  ...
  const IncidentCard = ({ incident, actions }) => {
    ...
    return (
      ...
      >
        ...
        {assignedVol && (
          <div className="flex items-center gap-1.5 mb-2 bg-zinc-800/30 rounded px-2 py-1">
        ...
        {/* Resolved timestamp */}
        {incident.resolvedAt && (
          <div className="mt-1.5 pt-1.5 border-t border-slate-800/30 text-[9px] text-emerald-600 font-mono flex items-center gap-1">
  ...
  const Column = ({ icon: Icon, iconColor, title, count, children }) => (
    <div className="bg-zinc-950/60 flex flex-col min-h-0">
      <div className="flex-shrink-0 px-3 py-2 border-b border-slate-800/30 flex items-center justify-between">
        ...
        <span className="text-[10px] font-mono text-slate-600 bg-slate-800/40 px-1.5 py-0.5 rounded">
          {count}
        </span>
      </div>
  ...
  return (
    <div className="glass-card flex flex-col overflow-hidden relative">
      ...
      {/* Three-column grid */}
      <div className="flex-1 grid grid-cols-3 gap-px bg-slate-800/20 min-h-0 overflow-hidden">
        {/* Column A: Unassigned */}
        <Column
          icon={AlertTriangle}
            ...
            <IncidentCard
              ...
              actions={
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-indigo-600/80 hover:bg-indigo-500 rounded text-[9px] font-bold transition-all active:scale-95"
                >
            ...
            <IncidentCard
              ...
              actions={
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-emerald-600/80 hover:bg-emerald-500 rounded text-[9px] font-bold transition-all active:scale-95"
                >
                  <CheckCircle2 className="w-2.5 h-2.5" />
                  Mitigate
                </button>
      ...
      {dispatchModal && (
        <div
          className="absolute inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in"
          onClick={() => setDispatchModal(null)}
        >
          <div
            className="bg-zinc-900 border border-slate-700/50 rounded-xl p-5 w-[440px] shadow-2xl shadow-black/50 animate-slide-up"
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => setDispatchModal(null)}
                className="p-1 text-slate-500 hover:text-slate-300 transition rounded hover:bg-slate-800/50"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {nearestVolunteers.length > 0 ? (
              <div className="space-y-2">
                {nearestVolunteers.map((v, i) => (
                  <div
                    key={v.id}
                    className="flex items-center justify-between bg-zinc-800/50 rounded-lg px-3 py-2.5 border border-slate-700/30 hover:border-indigo-500/30 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div className="w-10 h-10 rounded-full bg-emerald-500/12 border-2 border-emerald-500/35 flex items-center justify-center">
                          <span className="text-xs font-extrabold text-emerald-400">
                            {v.initials}
                          </span>
                        </div>
                        <span className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-indigo-600 flex items-center justify-center text-[8px] font-bold text-white">
                          {i + 1}
                        </span>
                      </div>
                      <div>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-[10px] font-bold transition-all duration-200 hover:shadow-lg hover:shadow-indigo-500/20 active:scale-95"
                    >
                      <Navigation className="w-3 h-3" />
                      DISPATCH
                    </button>
